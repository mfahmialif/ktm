            ...
            <form action="{{ route('students.update', $student) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="nik" id="nik" value="{{ old('nik', $student->nik) }}" class="w-full px-4 py-2.5 text-sm border rounded-lg focus:ring-2 focus:ring-primary focus:border-primary dark:bg-[#23303d] dark:border-[#2a3b4d] dark:text-white @error('nik') border-red-500 @enderror" placeholder="16 digit NIK" maxlength="16" />
                        @error('nik')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
