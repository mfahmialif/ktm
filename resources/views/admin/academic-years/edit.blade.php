<x-admin-layout>
    ...
    <div class="max-w-2xl">
        ...
        <!-- Page Header -->
        <div class="mb-6">
            <h2 class="text-2xl font-bold tracking-tight text-[#111418] dark:text-white">Edit Tahun Akademik</h2>
            <p class="mt-1 text-sm text-[#617589] dark:text-slate-400">Ubah data tahun akademik.</p>
        </div>
            ...
            <form action="{{ route('academic-years.update', $academicYear) }}" method="POST">
                ...
                <div class="p-6 space-y-5">
                    ...
                    <div>
                        ...
                        <input type="text" name="code" id="code" value="{{ old('code', $academicYear->code) }}" placeholder="Contoh: 20241" class="w-full px-4 py-2.5 text-sm border rounded-lg focus:ring-2 focus:ring-primary focus:border-primary dark:bg-[#23303d] dark:border-[#2a3b4d] dark:text-white @error('code') border-red-500 @enderror" />
                        @error('code')
                    ...
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $academicYear->name) }}" placeholder="Contoh: 2024/2025 Ganjil" class="w-full px-4 py-2.5 text-sm border rounded-lg focus:ring-2 focus:ring-primary focus:border-primary dark:bg-[#23303d] dark:border-[#2a3b4d] dark:text-white @error('name') border-red-500 @enderror" />
                        @error('name')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="flex items-center gap-3">
                        <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $academicYear->is_active) ? 'checked' : '' }} class="w-4 h-4 text-primary bg-gray-100 border-gray-300 rounded focus:ring-primary dark:bg-[#23303d] dark:border-[#2a3b4d]" />
